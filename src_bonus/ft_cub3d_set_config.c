...
/*   Updated: 2026/07/29 05:16:30 by rselva-2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	ctx->config.max_fps.current = fmin(fmax(max_fps, 1), ctx->config.max_fps.max);
	ctx->usec_per_frame = 1000000 / ctx->config.max_fps.current;
}

void	set_velocity(t_context *ctx, int veloc_lvl)
{
	ctx->config.veloc_lvl.current = fmin(fmax(veloc_lvl, 0), ctx->config.veloc_lvl.max);
	ctx->player.velocity = (double)1 / (ctx->config.veloc_lvl.max - (veloc_lvl) + 1);
	return ;
	ctx->player.velocity = 1;
	while (veloc_lvl <= ctx->config.veloc_lvl.max)
	{
		ctx->player.velocity /= 2;
		veloc_lvl++;
	}
}
...
{
	ctx->config.rot_veloc_lvl.current = fmin(fmax(rot_veloc_lvl, 0), ctx->config.rot_veloc_lvl.max);
	ctx->player.rotation_velocity = M_PI;
	while (rot_veloc_lvl <= ctx->config.rot_veloc_lvl.max)
	{
		ctx->player.rotation_velocity *= 0.5;
		rot_veloc_lvl++;
	}
}
...
{
	ctx->config.mouse_sens.current = fmin(fmax(rot_veloc_lvl, 0), ctx->config.mouse_sens.max);
	ctx->player.mouse_sensitivity = M_PI / 32;

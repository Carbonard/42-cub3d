...
// 	for (int i = -a; i < a; i++)
// 		for (int j = -a; j < a; j++)
// 			put_pixel(&map->img, (int)(x  * map->minimap_scale + i),
// (int)(y * map->minimap_scale + j), color);
// }
...
// 		i += 0.01;
// 	}
// 	put_pixel(&ctx->map.img, (int)(p.x * ctx->map.minimap_scale),
// (int)(p.y * ctx->map.minimap_scale), rgb(255,0,0));
// }
